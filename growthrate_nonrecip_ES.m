%Contains code to find parasite ES virulence on hosts with a non-reciprocal relationship. 
%Case 2 host specialism (Growth Rate); parasite grows faster (epsilon) on PH (host 1) than NPH (host 2).
%Code provided for both symmetric and asymmetric non-reciprocal relationships. 
...
%range of epsilon tested to find ES virulence
e_min = 3;
e_max = 5;
stepsize = 0.05;
vec = e_min:stepsize:e_max;
Fvec = [0.82, 0.84, 0.86, 0.88];
...
%r21vec = [0.0002*ones(1,3), 0, -0.0002*ones(1,3)]; %for asymmetric
%interactions
